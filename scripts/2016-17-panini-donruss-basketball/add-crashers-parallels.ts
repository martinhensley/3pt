...
async function addCrashersParallels() {
  try {
    console.log('Adding Crashers parallels...\n');
    ...
    // 2. Find the base "Crashers" set to get the cards
    const baseCrashersSet = await prisma.set.findFirst({
      where: {
    ...
    if (!baseCrashersSet) {
      console.error('Base "Crashers" set not found!');
      return;
    }

    console.log(`Found base set with ${baseCrashersSet.cards.length} cards\n`);
    ...
    for (const parallel of PARALLELS) {
      ...
      let cardsCreated = 0;
      for (const baseCard of baseCrashersSet.cards) {
      ...
      console.log(`  ✅ Created ${cardsCreated}/${baseCrashersSet.cards.length} cards\n`);
    }

    // 4. Summary
    console.log('='.repeat(60));
    console.log(`✅ Successfully added ${totalSetsCreated} Crashers parallel sets`);
    console.log(`✅ Created ${totalCardsCreated} cards`);
    console.log('='.repeat(60));

    // 5. Validation
    console.log('\n===== Validation =====');
    const crashersSets = await prisma.set.findMany({
      where: {
        releaseId: release.id,
        name: { contains: 'Crashers' }
      },
    ...
    console.log(`\nTotal "Crashers" sets: ${crashersSets.length}`);
    crashersSets.forEach(set => {
    ...
    console.error('Error adding Crashers parallels:', error);
    throw error;
  } finally {
