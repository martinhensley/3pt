...
async function addHallKingsParallels() {
  try {
    console.log('Adding Hall Kings parallels...\n');
    ...
    // 2. Find the base "Hall Kings" set to get the cards
    const baseHallKingsSet = await prisma.set.findFirst({
      where: {
    ...
    if (!baseHallKingsSet) {
      console.error('Base "Hall Kings" set not found!');
      return;
    }

    console.log(`Found base set with ${baseHallKingsSet.cards.length} cards\n`);
    ...
    for (const parallel of PARALLELS) {
      ...
      let cardsCreated = 0;
      for (const baseCard of baseHallKingsSet.cards) {
      ...
      console.log(`  ✅ Created ${cardsCreated}/${baseHallKingsSet.cards.length} cards\n`);
    }

    // 4. Summary
    console.log('='.repeat(60));
    console.log(`✅ Successfully added ${totalSetsCreated} Hall Kings parallel sets`);
    console.log(`✅ Created ${totalCardsCreated} cards`);
    console.log('='.repeat(60));

    // 5. Validation
    console.log('\n===== Validation =====');
    const hallKingsSets = await prisma.set.findMany({
      where: {
        releaseId: release.id,
        name: { contains: 'Hall Kings' }
      },
    ...
    console.log(`\nTotal "Hall Kings" sets: ${hallKingsSets.length}`);
    hallKingsSets.forEach(set => {
    ...
    console.error('Error adding Hall Kings parallels:', error);
    throw error;
  } finally {
