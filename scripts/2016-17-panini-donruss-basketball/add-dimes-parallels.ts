...
async function addDimesParallels() {
  try {
    console.log('Adding Dimes parallels...\n');
    ...
    // 2. Find the base "Dimes" set to get the cards
    const baseDimesSet = await prisma.set.findFirst({
      where: {
    ...
    if (!baseDimesSet) {
      console.error('Base "Dimes" set not found!');
      return;
    }

    console.log(`Found base set with ${baseDimesSet.cards.length} cards\n`);
    ...
    for (const parallel of PARALLELS) {
      ...
      let cardsCreated = 0;
      for (const baseCard of baseDimesSet.cards) {
      ...
      console.log(`  ✅ Created ${cardsCreated}/${baseDimesSet.cards.length} cards\n`);
    }

    // 4. Summary
    console.log('='.repeat(60));
    console.log(`✅ Successfully added ${totalSetsCreated} Dimes parallel sets`);
    console.log(`✅ Created ${totalCardsCreated} cards`);
    console.log('='.repeat(60));

    // 5. Validation
    console.log('\n===== Validation =====');
    const dimesSets = await prisma.set.findMany({
      where: {
        releaseId: release.id,
        name: { contains: 'Dimes' }
      },
    ...
    console.log(`\nTotal "Dimes" sets: ${dimesSets.length}`);
    dimesSets.forEach(set => {
    ...
    console.error('Error adding Dimes parallels:', error);
    throw error;
  } finally {
