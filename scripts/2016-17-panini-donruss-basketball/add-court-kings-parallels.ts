...
async function addCourtKingsParallels() {
  try {
    console.log('Adding Court Kings parallels...\n');
    ...
    // 2. Find the base "Court Kings" set to get the cards
    const baseCourtKingsSet = await prisma.set.findFirst({
      where: {
    ...
    if (!baseCourtKingsSet) {
      console.error('Base "Court Kings" set not found!');
      return;
    }

    console.log(`Found base set with ${baseCourtKingsSet.cards.length} cards\n`);
    ...
    for (const parallel of PARALLELS) {
      ...
      let cardsCreated = 0;
      for (const baseCard of baseCourtKingsSet.cards) {
      ...
      console.log(`  ✅ Created ${cardsCreated}/${baseCourtKingsSet.cards.length} cards\n`);
    }

    // 4. Summary
    console.log('='.repeat(60));
    console.log(`✅ Successfully added ${totalSetsCreated} Court Kings parallel sets`);
    console.log(`✅ Created ${totalCardsCreated} cards`);
    console.log('='.repeat(60));

    // 5. Validation
    console.log('\n===== Validation =====');
    const courtKingsSets = await prisma.set.findMany({
      where: {
        releaseId: release.id,
        name: { contains: 'Court Kings' }
      },
    ...
    console.log(`\nTotal "Court Kings" sets: ${courtKingsSets.length}`);
    courtKingsSets.forEach(set => {
    ...
    console.error('Error adding Court Kings parallels:', error);
    throw error;
  } finally {
