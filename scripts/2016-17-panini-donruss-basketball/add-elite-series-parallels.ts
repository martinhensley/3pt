...
async function addEliteSeriesParallels() {
  try {
    console.log('Adding Elite Series parallels...\n');
    ...
    // 2. Find the base "Elite Series" set to get the cards
    const baseEliteSeriesSet = await prisma.set.findFirst({
      where: {
    ...
    if (!baseEliteSeriesSet) {
      console.error('Base "Elite Series" set not found!');
      return;
    }

    console.log(`Found base set with ${baseEliteSeriesSet.cards.length} cards\n`);
    ...
    for (const parallel of PARALLELS) {
      const baseSetName = 'Elite Series';
      ...
      let cardsCreated = 0;
      for (const baseCard of baseEliteSeriesSet.cards) {
      ...
      console.log(`  ✅ Created ${cardsCreated}/${baseEliteSeriesSet.cards.length} cards\n`);
    }

    // 4. Summary
    console.log('='.repeat(60));
    console.log(`✅ Successfully added ${totalSetsCreated} Elite Series parallel sets`);
    console.log(`✅ Created ${totalCardsCreated} cards`);
    console.log('='.repeat(60));

    // 5. Validation
    console.log('\n===== Validation =====');
    const eliteSeriesSets = await prisma.set.findMany({
      where: {
        releaseId: release.id,
        name: { contains: 'Elite Series' }
      },
    ...
    console.log(`\nTotal "Elite Series" sets: ${eliteSeriesSets.length}`);
    eliteSeriesSets.forEach(set => {
    ...
    console.error('Error adding Elite Series parallels:', error);
    throw error;
  } finally {
