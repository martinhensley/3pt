...
async function fixKitSeriesPrintRuns() {
  try {
    console.log('Fixing Kit Series print runs...\n');
    ...
    // Update Kit Series Prime to /10 (most common print run)
    const primeSet = release.sets.find(s => s.name === 'Kit Series Prime');
    if (primeSet && primeSet.printRun !== 10) {
      console.log('Updating Kit Series Prime:');
      console.log('  Current: ' + (primeSet.printRun || 'null'));
      console.log('  New: /10 (variable /2 to /10)');

      await prisma.set.update({
        where: { id: primeSet.id },
        data: { 
          printRun: 10,
          description: 'Variable print runs ranging from /2 to /10 per player'
        }
      });
    ...
    // Update Kit Series Super Prime to 1/1
    const superPrimeSet = release.sets.find(s => s.name === 'Kit Series Super Prime');
    if (superPrimeSet && superPrimeSet.printRun !== 1) {
      console.log('\nUpdating Kit Series Super Prime:');
