...
export default function EbayAd({
  ...
  if (loading) {
    return (
      <div className="bg-white dark:bg-gray-800 border-2 border-footy-gold rounded-lg shadow-lg p-5 sticky top-4">
        <p className="text-sm text-footy-dark-green dark:text-footy-gold font-bold mb-4 text-center uppercase tracking-wide">
          {title}
        </p>
    ...
    <div className="bg-white dark:bg-gray-800 border-2 border-footy-gold rounded-lg shadow-lg p-5 sticky top-4">
      ...
      <div className="space-y-4">
        {products.map((product) => (
          ...
          >
            <div className="relative w-full h-48 bg-white dark:bg-gray-600">
              {product.image ? (
                <Image
                  src={product.image}
                  alt={product.title}
            ...
            <div className="p-3 bg-white dark:bg-gray-800">
              <p className="text-sm text-gray-900 dark:text-gray-100 font-semibold line-clamp-2 mb-2 leading-tight">
                {product.title}
              </p>
              <p className="text-lg font-bold text-footy-dark-green dark:text-footy-gold">
                {new Intl.NumberFormat("en-US", {
                  style: "currency",
                  currency: product.currency,
                }).format(parseFloat(product.price))}
              </p>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">View on eBay →</p>
            </div>
          </a>
        ))}
      </div>
